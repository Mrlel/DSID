...
<div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between py-4 gap-3">
    <div>
        ...
        <p class="text-muted mb-0">Tous les mouvements de véhicules de la direction</p>
    </div>
...
{{-- Recherche --}}
<form method="GET" action="{{ route('vehicules.historique') }}" class="card shadow-sm mb-4">
            ...
            <div class="col-md-5">
                ...
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control"
                           placeholder="Immatriculation ou nom utilisateur…"
                           value="{{ request('search') }}">
                </div>
            </div>
            <div class="col-md-2 d-flex gap-2">
                <button type="submit" class="btn btn-primary w-100">Filtrer</button>
                <a href="{{ route('vehicules.historique') }}" class="btn btn-outline-secondary w-100">Reset</a>
            </div>
        </div>
    </div>
</form>
    ...
    <table class="table table-striped table-hover align-middle mb-0">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    @if($a->vehicule)
                    <a href="{{ route('vehicules.show', $a->vehicule->id) }}" class="fw-semibold text-decoration-none">
                        {{ $a->vehicule->immatriculation }}
                    </a>
                    <div class="small text-muted">{{ $a->vehicule->marque }} {{ $a->vehicule->modele }}</div>
                    @else
