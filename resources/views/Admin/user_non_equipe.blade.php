...
@include('layouts.user-stat-card')
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                    @foreach ($users_non_equipes as $user_non_equipe)
                    <tr>
                       <td>
                        {{$user_non_equipe->nom}}
                        </td>
                        <td>{{$user_non_equipe->prenom ?? 'non spécifié'}}</td>
                        <td>{{$user_non_equipe->matricule ?? 'non spécifié'}}</td>
                        <td><i class="mail icon"></i> {{$user_non_equipe->email}}</td>
                        <td>{{$user_non_equipe->contact}}</td>
                        ...
                        <td class="text-center">
                            <div class="dropdown">
                                <button class="btn btn-sm btn-light dropdown-toggle" type="button" id="dropdownMenuButton{{ $user_non_equipe->id }}" data-bs-toggle="dropdown" aria-expanded="false">
                                    <i class="bi bi-three-dots-vertical"></i>
                                </button>

                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton{{ $user_non_equipe->id }}">
            ...
            @else
                <div class="text-center py-3 mt-4">
                    ...
                    <p class="text-muted">Aucun utilisateur non connecté</p>
                </div>
            @endif
        </div>
