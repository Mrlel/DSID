...
<header class="my-4">
  <h1 class="h3 fw-bold text-dark mb-1 d-flex align-items-center">
    Gestion des Logiciels Expirés 
  </h1>
  <p class="text-muted mb-0">
    Liste des logiciels dis • non valides
  </p>
</header>
 ...
 <div class="table-responsive">
       ...
       <table class="table table-striped">
                ...
                <tbody>
                    @foreach ($licencesExpirees as $licencesExpiree)
                    <tr>
                        <td>{{$licencesExpiree->designation_licence}}</td>
                        <td>{{$licencesExpiree->type_licence}}</td>
                        <td><i class="far fa-calendar-alt"></i> {{$licencesExpiree->date_expiration}}</td>
                        <td>{{$licencesExpiree->cle_licence}}</td>
                        <td>{{$licencesExpiree->environnement}}</td>
                        <td>{{$licencesExpiree->langage_version}}</td>
                        <td>{{$licencesExpiree->sgbd_version}}</td>
                        <td class="d-flex align-item-center gap-2">
                            <div class="item"><a class="text-danger" href="/delete_logiciel/{{$licencesExpiree->id}}" onClick="return confirm('Voulez-vous vraiment supprimer ce logiciel ?')"><ion-icon name="trash-outline"></ion-icon></a></div>
                            <div class="item"><a href="/logiciel/{{$licencesExpiree->id}}"><ion-icon name="information-circle-outline"></ion-icon></a></div>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
                ...
                <form class="ui form" action="{{ route('assigner_logiciels.store') }}" method="POST">
                    ...
                    <div class="field">
                        ...
                        <div class="ui fluid search selection dropdown">
                            ...
                            <div class="menu">
                                @foreach($licencesExpirees as $licencesExpiree)
                                    <div class="item" data-value="{{ $licencesExpiree->id }}">
                                        <i class="cube icon"></i>
                                        {{ $licencesExpiree->designation_licence }}
                                    </div>
                                @endforeach
                            </div>
                        </div>
